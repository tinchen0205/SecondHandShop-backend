...
app.get('/products', async (req, res) => {
  try {                                                 //改成你自己的
    const [rows] = await con.execute("SELECT id, DATE_FORMAT(delivery_datetime, '%Y-%m-%d') AS delivery_datetime, total_amount, DATE_FORMAT(return_datetime, '%Y-%m-%d') AS return_datetime FROM rental_orders");
    res.json(rows);
  } catch (error) {
